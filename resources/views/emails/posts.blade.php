...
@empty
    ...
    <div
        style="font-size: 120%; color: rgb(119, 117, 117);  padding:10px; background:  rgb(141, 5, 35); color:antiquewhite; margin-bottom:10px ">
        Podujatia
    </div>
    ...
    @forelse($events as $post)
        ...
        <div>
            <div style=" background: white; padding-bottom:2px; max-width: 700px; margin:auto; border-style: solid;
            border-color: rgb(141, 5, 35); margin-bottom: 10px ">



                @forelse($post->images as $image)
                    <img alt="{{ $post->organization->title }} / {{ $post->title }}"
                        src="{{ url($image->ThumbImageUrl) }}" style="width: 60px;">
                @break
            @empty
                @if (isset($post->user->avatar))
                    <img src="{{ Storage::url($post->user->userPictureUrl()) }}"
    ...
    <div style="margin-top: -10px; font-size: 90%; margin-bottom:20px ">Autor: {{ $post->organization->title }}</div>
</div>
</div>
@empty
bez záznamu
