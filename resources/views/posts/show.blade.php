...
@section('content')
    ...
    <header class="border-b border-[color:var(--ar-line)] bg-white">
        <div class="mx-auto max-w-6xl px-4 py-8 md:py-12">
            <div class="max-w-3xl">
                <a href="{{ $orgUrl }}" class="ar-kicker ar-link">{{ $post->organization->title }}</a>

                <h1 class="ar-display mt-3 text-3xl font-extrabold leading-tight md:text-[2.6rem]">
                    {{ $post->title }}
                </h1>
    ...
    <div class="mx-auto max-w-6xl px-4 py-8">
        ...
        <div class="grid gap-10 lg:grid-cols-12">
            ...
            <aside class="lg:col-span-4">
                <div class="space-y-4 lg:sticky lg:top-6">
                    ...
                    {{-- Ďalšie príspevky toho istého kanála --}}
                    @php $sidebarPosts = $posts->reject(fn ($item) => $item->id === $post->id)->take(5); @endphp
        ...
        {{-- Celý archív kanála --}}
        <section class="mt-14">
            <h2 class="ar-rule ar-display mb-5 text-lg font-bold">
            ...
            <div class="grid grid-cols-2 gap-4 md:grid-cols-4 lg:grid-cols-5">
                @forelse ($posts as $item)
                    <a href="{{ route('post.show', [$item->id, $item->slug]) }}"
                       class="ar-card block overflow-hidden rounded-lg">
                        <span class="relative block">
                            <img data-src="{{ $item->thumbImage }}" data-sizes="auto" alt="{{ $item->title }}"
                                 class="lazyload h-28 w-full object-cover">
                            @if ($item->video_duration)
                                <span class="absolute bottom-1 right-1 rounded bg-black/75 px-1.5 py-0.5 text-[.65rem] text-white">
                                    {{ $item->video_duration }}
                                </span>
                            @endif
                        </span>
                        <span class="block p-3">
                            <span class="ar-display block text-sm font-semibold leading-snug">
                                {{ Str::limit($item->title, 52) }}
                            </span>
                ...
                @empty
                    <p class="text-sm text-gray-500">Bez záznamu.</p>
                @endforelse
            </div>
