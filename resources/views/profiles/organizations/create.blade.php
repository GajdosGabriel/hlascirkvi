...
@section('content')
    <x-dashboard.frame>
        <x-dashboard.header heading="Nový kanál">
            ...
            <x-slot name="actions">
                <a class="btn btn-default" href="{{ route('profile.user.organization.index', $user) }}">Späť na kanály</a>
            </x-slot>
        </x-dashboard.header>

        @include('layouts.errors')

        <form method="POST" action="{{ route('profile.user.organization.store', $user) }}" class="space-y-6">
            @csrf
            ...
            <div class="flex flex-wrap justify-end gap-3">
                <a class="btn btn-default" href="{{ route('profile.user.organization.index', $user) }}">Zrušiť</a>
